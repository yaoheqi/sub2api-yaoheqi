...
func (s *OpenAIGatewayService) updateCodexUsageSnapshot(ctx context.Context, accountID int64, snapshot *OpenAICodexUsageSnapshot) {
	...
	// Route every snapshot through the overdraft-aware path. Besides persisting
	// the snapshot, it evaluates the prearm/exhaustion signal and starts the
	// bounded probe when the account enters a quota cycle.
