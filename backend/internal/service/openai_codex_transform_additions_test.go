...
// applyCodexClientMetadata：用部署密钥派生 installation 标识，幂等、不覆盖既有项。
func TestApplyCodexClientMetadata(t *testing.T) {
	...
	body := map[string]any{}
	require.True(t, applyCodexClientMetadata(body, acc, "deployment-a"))
	cm, ok := body["client_metadata"].(map[string]any)
	require.True(t, ok)
	require.NotEqual(t, "dev-xyz", cm["x-codex-installation-id"])
	// 幂等
	require.False(t, applyCodexClientMetadata(body, acc, "deployment-a"))
	...
	require.True(t, applyCodexClientMetadata(body3, acc, "deployment-a"))
	cm3, _ := body3["client_metadata"].(map[string]any)
	require.Equal(t, "t", cm3["x-codex-turn-metadata"])
	require.NotEqual(t, "dev-xyz", cm3["x-codex-installation-id"])
}
