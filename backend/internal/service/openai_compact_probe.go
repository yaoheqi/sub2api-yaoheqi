...
// compactProbeSessionID preserves the legacy compact probe identity used by
// non-OAuth accounts. OAuth-only protocol shaping is exposed separately below
// so API-key probes do not change their existing wire identifiers.
...
// compactProbeOAuthSessionID uses the UUIDv7-shaped identifier required by
// the Codex OAuth /responses probe. Keep the distinction explicit at the call
// site rather than changing the shared helper for API-key accounts.
func compactProbeOAuthSessionID(accountID int64) string {
	if accountID <= 0 {
		return deriveStableUUIDv7("sub2api:codex-compact-probe:v1:anonymous")
	}
	return deriveStableUUIDv7("sub2api:codex-compact-probe:v1:" + strconv.FormatInt(accountID, 10))
}
